...
** Toggle line wrap used especially for writing documents or blog posts.

Got this code snippet from [[http://ergoemacs.org/emacs/emacs_novel_reading_mode.html]] with a few changes.

#+BEGIN_SRC emacs-lisp
  (defun as/toggle-line-wrap ()
    "Toggle line wrap used expecially for writing documents or blog
  posts.

    Calling it will toggle it back to its former state"
    (interactive)
    (let ()
