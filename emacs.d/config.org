...
** TODO diminish eldoc
** TODO Dominion Flyspell/flycheck
** TODO disable flyspell for global mode. only trigger when needed.
** TODO troubleshoot slowness when editing ReactJS file
** TODO find how to change the font colour of the current date in the org agenda view
** TODO Add more remarks/comments/notes for each =emacs-lisp= code
** TODO do not load theme when emacs is run in terminal/cmd
...
#+BEGIN_SRC emacs-lisp
	(use-package dashboard :tangle no
		:disabled
		:after projectile
		:config
...
A handy tool for drawing UML diagrams.
